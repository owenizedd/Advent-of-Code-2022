...
maxHeight = -1;
function parse() {
  maxHeight = inp.findIndex((str) => str[0] == 'm');
  for (let i = 0; i < maxHeight - 2; i++) {
    for (let j = 0; j < inp[i].length; j++) {
      c = inp[i][j]
      if (c !== '[' && c !== ']' && c !== ' ') {
        idx = inp[maxHeight - 2][j];
        data[idx].push(c);
      }
    }
  }
...
function move(amount, from, to) {
  let temp = [];
  for (let i = data[from].length - amount; i < data[from].length; i++) {
    temp.push(data[from][i])
  }
  temp.forEach((c) => {
    data[to].push(c);
  })
  data[from].splice(data[from].length - amount, amount);
}
function solve() {
  for (let i = maxHeight; i < inp.length; i++) {
    const moves = inp[i].replace("move", "").replace("from", "").replace("to", "").trim().split(' ').filter(m => m).map(m => +m)
    const [amount, from, to] = moves;
